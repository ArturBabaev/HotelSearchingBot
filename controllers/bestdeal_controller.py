...
from service.bestdeal import bestdeal_hotels
...
class BestdealController:
    """
    Best deal request processing class
    """

    def __init__(self, bot: TeleBot, user_repository: UserRepositoryDB,
                 history_repository: HistoryRepositoryDB) -> None:
        self.bot = bot
        self.user_repository = user_repository
        self.history_repository = history_repository

    def process_bestdeal(self, message: types.Message) -> None:
        """
        The method calls a method choose_language and passes a message and text
        """

        text = 'Вы выбрали команду /bestdeal.\nВыберите язык для ввода названия города.'
        ...
        try:
            ...
            logger.info('user id: {}, command: {}', message.chat.id, '/bestdeal')
    ...
    def choose_price_min(self, message: types.Message) -> None:
        """
        The method for price min selection
        """

        text = 'Введите минимальную цену проживания в сутки в руб.'

        self.bot.send_message(message.chat.id, text=text)

        self.bot.register_next_step_handler(message, self.price_min_callback)

    def price_min_callback(self, message: types.Message) -> None:
        """
        The method for price min callback
        """

        user = self.user_repository.get_user(message.chat.id)

        try:
            user.price_min = float(message.text.replace(',', '.'))

            if 2 ** 40 < user.price_min or user.price_min <= 0:
                raise ValueError('invalid data')

            self.user_repository.set_user(user)

            self.choose_price_max(message)

        except ValueError as error:
            logger.info('user id: {}, command: {}', message.chat.id, user.command)
            logger.error('error: {} \n{}', error, traceback.format_exc())

            self.bot.send_message(message.chat.id, 'Проверьте корректность вводимых данных! '
                                                   'Значение должно быть больше нуля и введено цифрами.')

            return self.choose_price_min(message)

    def choose_price_max(self, message: types.Message) -> None:
        """
        The method for price max selection
        """

        text = 'Введите максимальную цену проживания в сутки в руб.'

        self.bot.send_message(message.chat.id, text=text)

        self.bot.register_next_step_handler(message, self.price_max_callback)

    def price_max_callback(self, message: types.Message) -> None:
        """
        The method for price max callback
        """

        user = self.user_repository.get_user(message.chat.id)

        try:
            user.price_max = float(message.text.replace(',', '.'))

            if 2 ** 40 < user.price_max or user.price_max <= 0:
                raise ValueError('invalid data')

            self.user_repository.set_user(user)

            if user.price_max < user.price_min:
                self.bot.send_message(message.chat.id, 'Вы перепутали вводимые значения местами, но ничего страшного '
                                                       'я сам все исправлю. Продолжайте ввод.')

            self.choose_distance_min(message)
            ...
            return self.choose_price_max(message)

    def choose_distance_min(self, message: types.Message) -> None:
        """
        The method for distance min selection
        """

        text = 'Введите минимальное расстояние от центра в км.'

        self.bot.send_message(message.chat.id, text=text)

        self.bot.register_next_step_handler(message, self.distance_min_callback)

    def distance_min_callback(self, message: types.Message) -> None:
        """
        The method for distance min callback
        """

        user = self.user_repository.get_user(message.chat.id)

        try:
            user.distance_min = float(message.text.replace(',', '.'))

            if 2 ** 40 < user.distance_min or user.distance_min <= 0:
                raise ValueError('invalid data')

            self.user_repository.set_user(user)

            self.choose_distance_max(message)
            ...
            return self.choose_distance_min(message)

    def choose_distance_max(self, message: types.Message) -> None:
        """
        The method for distance max selection
        """

        text = 'Введите максимальное расстояние от центра в км.'

        self.bot.send_message(message.chat.id, text=text)

        self.bot.register_next_step_handler(message, self.distance_max_callback)

    def distance_max_callback(self, message: types.Message) -> None:
        """
        The method for distance max callback
        """

        user = self.user_repository.get_user(message.chat.id)

        try:
            user.distance_max = float(message.text.replace(',', '.'))

            if 2 ** 40 < user.distance_max or user.distance_max <= 0:
                raise ValueError('invalid data')

            self.user_repository.set_user(user)

            if user.distance_max < user.distance_min:
                self.bot.send_message(message.chat.id, 'Вы перепутали вводимые значения местами, но ничего страшного '
                                                       'я сам все исправлю. Продолжайте ввод.')

            self.choose_hotel_qty(message)
            ...
            return self.choose_distance_max(message)
    ...
    def choose_hotel(self, message: types.Message) -> None:
        ...
        try:
            ...
            distance_min = user.distance_min
            distance_max = user.distance_max
            price_min = int(user.price_min)
            price_max = int(user.price_max)
            check_in = user.check_in
            check_out = user.check_out
            days_stay = (datetime.date.fromisoformat(check_out) - datetime.date.fromisoformat(check_in))

            hotels_list = bestdeal_hotels(city_id, loc_lang, currency, hotels_qty, distance_min, distance_max,
                                          price_min, price_max, check_in, check_out, days_stay.days,
                                          page_number=1, bestdeal_hotels_list=[])

            user.hotels_list = hotels_list

            self.user_repository.set_user(user)

            self.history_repository.set_history(History(user_id=message.chat.id, command='/bestdeal',
            ...
            if user.hotels_list:
                ...
                self.bot.edit_message_text(text='На сайте Hotels.com отсутствуют гостиницы с данными параметрами.',
                                           chat_id=message.chat.id, message_id=msg.message_id)
